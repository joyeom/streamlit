...
def check_redacted(df):
    ...
    df["redacted_error_t1"] = calculate_red_error(df['redacted_matched_og'],df['redacted_matched_t1'])

    if "Translation2" in df.columns:
      df["redacted_matched_t2"] = df["Translation2"].apply(extract_redacted)
      df["redacted_error_t2"] = calculate_red_error(df['redacted_matched_og'],df['redacted_matched_t2'])
