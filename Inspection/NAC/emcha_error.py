...
from Inspection.NAC.red_error import red_pattern 
...
def find_common_words(og, trans):
    ...
    for og, tran in zip(og, trans):
        ...
        if common_words:  # Check if there are common words
          common_words = [t for t in common_words if not (is_number(t) or is_website_link(t) or is_metric(t) or is_punctuation(t) or len(t) <= 1)]
...
def get_emcha(df,src,tg): #src = source language, tg = target language
  removed_og = remove_re(df["Origin"])
  removed_t1 = remove_re(df["Translation1"])
  ...
  if "Translation2" in df.columns:
    removed_t2 = remove_re(df["Translation2"])
