...
def tokenize(text,lang): #딱히 필요 없이 word_tokenize 쓰면 될꺼 같은데..? #chinese 는 꼭 jieba
  if lang == "vi":
  ...
  #   tokens = text.apply(lambda t: tokenize_korean(t))
    ...
    tokens = text.apply(lambda t: tokenize_english(t)) #if it doesn't have specific targeted open source just use word_tokenize from nltk
...
def is_metric(text):
    ...
    metric_pattern = r'\b(kg|g|mg|km|cm|mm|m|ml|l|cm\^2|m\^2|mm\^2|cm\^3|m\^3|mm\^3|kph|mph)\b'
...
def find_common_words(og, trans):
    ...
    for og, tran in zip(og, trans):
        ...
        if common_words:  # Check if there are common words
          common_words = [t for t in common_words if not (is_nan(t) or is_number(t) or is_website_link(t) or is_metric(t) or is_punctuation(t) or len(t) <= 1)]
...
def get_emcha(df,src,tg): #src = source language, tg = target language
  ...
  df["emcha_t1"] = df["emcha_t1"].apply(lambda x:', '.join(x) if isinstance(x,list) else np.nan)

  if contains_t2(df):
    removed_t2 = remove_re(df.iloc[:,4])
    tokenized_t2 = tokenize(removed_t2,tg)
    df["emcha_t2"] = find_common_words(tokenized_og,tokenized_t2)
    df["emcha_t2"] = df["emcha_t2"].apply(lambda x:', '.join(x) if isinstance(x,list) else np.nan)
