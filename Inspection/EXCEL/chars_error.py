...
import re
import numpy as np
from Inspection.EXCEL.emcha_error import emoji_pattern
...
def calculate_chars_error(og, t):
    ...
    for c_og, c_t in zip(og, t):
        if pd.isna(c_t):
            ...
            if (
                c_og == "" and c_t == ""
            ):  # check if it is just empty string since it is no need to check
                chars_error.append(False)
                ...
                chars_error.append(
                    c_og == c_t
                )  # if it has same content then make it TRUE
    return chars_error
